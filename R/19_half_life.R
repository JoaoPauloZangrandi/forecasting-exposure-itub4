...
# reversão à média. AR(1) no nível da posição (ITUB4): pos_t = c + rho*pos_{t-1}.
# rho<1 => reverte; meia-vida = ln(0.5)/ln(rho) meses (tempo p/ fechar metade do
# desvio em relacao ao alvo mu_g = c/(1-rho)). Confirma round 3 e dá um numero
# economico interpretavel. Tambem testa o peso (esperado ~RW, sem reversao).
# =============================================================================
...
write_tab(rbind(pos$tab, pes$tab), "half_life_by_gestora.csv")
