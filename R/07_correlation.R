...
# sfx: "" para a versão primária (net) -> nomes genéricos; "_direta"/"_long" p/ as demais.
build_correlations <- function(panel, sfx = "") {
  ...
  write_matrix(cor_pw, sprintf("cor_gestoras_delta_usd_pairwise%s.csv", sfx))
  write_matrix(n_pw,   sprintf("cor_gestoras_delta_usd_n%s.csv", sfx))
  ...
  p <- ggplot(cor_dt, aes(g1, g2, fill = corr)) +
    ...
    labs(title = sprintf("Correlacao entre gestoras: variacao mensal da posicao em ITUB4 (US$, %s)", lbl),
         x = NULL, y = NULL, fill = "corr") +
    theme_minimal(base_size = 8) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  ggsave(file.path(OUT_FIG, sprintf("correlation_heatmap_delta_usd%s.png", sfx)), p,
         width = 9, height = 8, dpi = 150)
  invisible(cor_pw)
}

plot_positions <- function(panel, sfx = "", top_n = 8L) {
  lbl <- if (sfx == "") "net" else sub("^_", "", sfx)
  ...
  p <- ggplot(panel[gestora %in% top], aes(data, pos_usd_mil / 1e3, color = gestora)) +
    ...
    labs(title = sprintf("Posicao em ITUB4 por gestora (US$ milhoes, exposicao %s)", lbl),
