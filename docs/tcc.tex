...
\vspace{2.5cm}
{\huge\bfseries Forecasting da exposição e da demanda de gestoras por ações\par}
\vspace{0.8cm}
...
\emph{demand-based asset pricing}. O núcleo empírico usa bases públicas da CVM, a composição consolidada de
carteiras (CONS) e a série histórica dos fundos (SH), para construir um painel mensal gestora $\times$ mês,
consolidado por grupo econômico, com validação inicial em ITUB4 e extensão para todas as ações identificáveis
no período 2016--2021. A exposição em valor (R\$ e US\$) mede escala patrimonial e risco. Para aproximar
demanda, este trabalho constrói também uma medida de quantidade estimada de ações, dividindo o valor de ITUB4
pelo fechamento mensal oficial da B3/COTAHIST.

Os resultados mostram que a distinção entre valor e quantidade é substantiva. Em valor, o nível da posição em
ITUB4 apresenta reversão à média e o AR reduz o RMSE em aproximadamente 10\% em relação ao \emph{random
walk}. Em quantidade estimada de ações, entretanto, a previsibilidade é bem menor: a variação mensal continua
próxima de um \emph{random walk} e o ganho no nível aparece apenas no horizonte de um mês, com cerca de 3,5\%
de redução de RMSE. Assim, parte da previsibilidade em valor parece refletir marcação a mercado e reversão
patrimonial, não necessariamente compras e vendas previsíveis de ações. Como apêndice metodológico, o
trabalho documenta uma extensão com a CDA Bloco~2, que reconstrói relações fundo$\to$fundo e estima dupla
contagem intra-gestora em estruturas FIC/master. Essa extensão é útil para discutir risco estrutural, mas fica
separada do núcleo CONS+SH+B3 até validação explícita com o orientador.
...
nasce de uma ideia simples: se investidores institucionais têm demandas persistentes por ativos, então as
carteiras observadas podem conter informação sobre comportamento futuro. A literatura de
...
ao seu patrimônio líquido. Essas duas bases, porém, informam valores de posição, não quantidades de ações.
Por isso, o trabalho separa duas medidas. A primeira é a exposição em valor, útil para escala econômica,
risco patrimonial e comparação em US\$. A segunda é a quantidade estimada de ações, construída para ITUB4 a
partir do preço de fechamento da B3/COTAHIST \citep{b3_cotahist}. Essa segunda medida é a aproximação mais
direta da demanda por ações.
...
\item Como transformar a posição em valor de ITUB4 em quantidade estimada de ações, separando efeito de
preço de mudança de demanda?
\item A variação mensal da quantidade de ITUB4 é previsível?
\item O nível da quantidade de ITUB4 reverte a uma alocação-alvo?
...
\item conversão de valor de ITUB4 para quantidade estimada de ações com fechamento mensal B3/COTAHIST;
...
Para estudar demanda, valor monetário não é suficiente. A posição em reais varia tanto por compra e venda de
ações quanto por variação de preço. Como a CONS não traz número de ações, a quantidade é estimada por:
\[
q_{g,t}^{ITUB4} =
...
Essa quantidade deve ser lida como quantidade estimada, não observada diretamente. Ela é a melhor aproximação
possível com CONS+B3, mas eventos societários que alterem mecanicamente o número de ações exigem cautela na
interpretação temporal. No recorte empírico, a vantagem central continua sendo separar efeito de preço de
mudança aproximada de demanda.
...
estimados somente na janela de treino, sem usar informação futura. A análise em valor é mantida para escala
econômica; a análise em quantidade estimada é tratada como especificação principal de demanda para ITUB4.
...
exercício em quantidade estimada de ações, que é a variável mais próxima de demanda, a conclusão se mantém e
fica ainda mais clara. O melhor modelo, AR encolhido, melhora apenas 0,6\% no RMSE e piora no MAE. A direção
fica em torno de 50\%.
...
A leitura é que a variação mensal da demanda aproximada se comporta como martingale. Esse resultado não é um
...
Quando o alvo é o nível da quantidade estimada de ações, a evidência é mais fraca. O AR individual melhora o
...
O projeto entrega quatro conclusões principais. Primeiro, é possível construir um painel mensal coerente de
exposição das gestoras a ações usando CONS e SH, com tratamento cuidadoso de formato numérico, empréstimos de
ações, grupos econômicos e PTAX. Segundo, para ITUB4, é possível transformar exposição em valor em quantidade
estimada de ações usando fechamento mensal oficial da B3/COTAHIST, com auditoria numérica direta. Terceiro,
a variação mensal da quantidade estimada é difícil de prever; o \emph{random walk} é um benchmark forte.
Quarto, a previsibilidade em valor é maior do que a previsibilidade em quantidade, o que mostra que parte da
reversão observada em US\$ vem de dinâmica patrimonial e de preço, não necessariamente de demanda previsível
por ações.

A interpretação econômica fica mais precisa. A demanda institucional medida em número aproximado de ações
muda de forma ruidosa no curto prazo. Já a exposição em valor de algumas ações muito líquidas e amplamente
detidas parece retornar a níveis patrimoniais de referência. Essa distinção é central para
\emph{demand-based asset pricing}: se o objeto é demanda, quantidade deve ser privilegiada; se o objeto é
risco patrimonial, valor é a medida apropriada. Esses mesmos papéis continuam relevantes para a discussão de
fragilidade, pois concentram demanda institucional sobreposta.
