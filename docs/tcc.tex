...
janela mínima de treino é de 36 meses. Tickers com \emph{random walk} perfeito na amostra fora da amostra,
como eventos societários ou séries degeneradas, são excluídos da métrica de \emph{skill}, pois o denominador
do ganho relativo é zero. O AR individual é estimado como reversão à média e tem coeficiente de persistência
limitado a $[0,0{,}995]$, para evitar extrapolações explosivas em séries curtas. Para avaliar significância
agregada, o projeto usa \emph{bootstrap} por data de origem, preservando a dependência transversal entre
gestoras e ações no mesmo mês.
...
estatisticamente robusta de nenhum modelo contra o \emph{random walk}.
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Modelo} & \textbf{RMSE} & \textbf{MAE} & \textbf{Skill RMSE} & \textbf{Direção} \\
\midrule
Random walk     & 10.752 & 2.638 & 0,00\%  & -- \\
AR1 painel      & 10.855 & 2.786 & -0,96\% & 51,9\% \\
$n-1$ painel    & 10.869 & 2.869 & -1,09\% & 51,0\% \\
AR1 individual  & 10.870 & 2.821 & -1,10\% & 51,9\% \\
Fator comum     & 10.874 & 2.878 & -1,14\% & 51,0\% \\
\bottomrule
\end{tabular}
\end{table}
